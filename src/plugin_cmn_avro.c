  ...
  struct timeval *basetime, struct pkt_stitching *stitch, avro_value_iface_t *iface)
{
  ...
  if (wtc_2 & COUNT_MPLS_LABEL_STACK) {
    ...
    if (config.nfacctd_mpls_label_stack_encode_as_array) {
      compose_nfacctd_mpls_label_stack_data(pmpls->labels_cicle, label_buf, value);
    } 
    else {
      int idx_0;
...
int compose_nfacctd_mpls_label_stack_data(u_int32_t *labels_cicle, char *label_buf, avro_value_t v_type_record)
{
  avro_value_t v_type_array, v_type_string;
